...
//  1. Uses new CATEGORY_TREE (13 real categories, no sendpickup)
//  2. Send & Pickup is a service shortcut button, NOT a category tile
//  3. CategoriesSection uses CAT_CONFIG from new tree
//  4. normCat updated to match new keys

import React, { useState, useEffect, useCallback, useRef } from "react";
import { useNavigate } from "react-router-dom";
import {
  FiShoppingCart, FiPackage, FiCoffee, FiStar,
  FiClock, FiTruck, FiChevronRight, FiZap,
  FiGrid, FiMapPin, FiBox, FiDroplet, FiHeart,
  FiSearch, FiRefreshCw, FiX, FiMinus, FiPlus,
  FiShield, FiTrendingUp, FiSend,
} from "react-icons/fi";
import {
...
import {
  RiSendPlaneFill, RiLeafLine, RiDrinks2Line,
  RiVerifiedBadgeFill, RiStore2Line,
} from "react-icons/ri";
...
const CAT: Record<string, { label: string; icon: React.ReactNode }> = {
  ...
  skincare:     { label: "Skincare",          icon: <FiDroplet size={24} /> },
  perfumes:     { label: "Perfumes",          icon: <FiDroplet size={24} /> },
  drinks:       { label: "Drinks",            icon: <RiDrinks2Line size={24} /> },
  health:       { label: "Health & Wellness", icon: <FiHeart size={24} /> },
  electronics:  { label: "Electronics",       icon: <FiBox size={24} /> },
};

// All real category IDs (used in grid and product sections)
const CATEGORY_IDS = Object.keys(CAT);
...
export type Product = {
  id: string; name: string; store: string; rating: number; price: string;
  img: string | null; category: string; vendorId: string;
  vendorName?: string; vendorVerified?: boolean;
  description?: string; highlights?: string; careInfo?: string;
  stock?: number; shipping?: RawProduct["shipping"];
};
type Trending = {
  name: string; rating: number; time: string; tag: string;
  img: string | null; categoryId: string; vendorId?: string;
};
type Address = {
  id: string; label: "Home" | "Work" | "Other"; address: string;
  landmark?: string; extraClue?: string; isDefault: boolean; lat?: number; lng?: number;
};
type VendorInfo = {
  name: string; logo?: string; verified?: boolean; rating?: number;
  reviewCount?: number; deliveryTime?: string; address?: string;
};
type AdProduct = {
  id: string; name: string; price?: string | number;
  img?: string | null; vendorId?: string; vendorName?: string; vendorVerified?: boolean;
};
...
const SIZE_LBL: Record<string, string> = {
  small: "Small pkg", medium: "Medium pkg", large: "Large pkg", extra_large: "Extra large",
};
const ESS = [
...
function normCat(raw: string): string {
  ...
  const map: [string, string][] = [
    ...
    ["food", "restaurants"],
    // sendpickup is a SERVICE — map it to restaurants to avoid orphan products
    ["logistics", "restaurants"], ["delivery", "restaurants"],
    ["courier", "restaurants"], ["send", "restaurants"], ["pickup", "restaurants"],
  ];
  for (const [k, v] of map) if (s.includes(k)) return v;
  return s in CAT ? s : "restaurants";
}
  ...
  try {
    ...
    snap.forEach(d => {
      ...
      out.push({
        ...
        img: bestImg(r),
        category: normCat(r.category || "restaurants"),
        vendorId: r.vendorId || "",
...
async function fetchVends(): Promise<Trending[]> {
  ...
  try {
    ...
    snap.forEach(d => {
      ...
      if (!name) return;
      const c = normCat(v.category || v.businessType || "restaurants");
      out.push({
        name,
        rating: typeof v.rating === "number" ? v.rating : +(4.5 + Math.random() * 0.5).toFixed(1),
        time: v.deliveryTime || `${10 + Math.floor(Math.random() * 20)}-${25 + Math.floor(Math.random() * 20)} mins`,
        tag: CAT[c]?.label || "Order Now",
        img: [v.coverImage, v.bannerImage, v.logo].find((u: any) => u && !u.includes("supabase.co")) ?? null,
        categoryId: c,
        vendorId: d.id,
      });
    });
  ...
  useEffect(() => {
    ...
    return onSnapshot(
      ...
      async snap => {
        ...
        setShuffled([...ads].sort(() => Math.random() - 0.5));
        setBannerAds(ads);
    ...
    <div style={{ marginBottom: 8 }}>
      <div style={{ display: "flex", gap: 12, padding: "0 18px", overflowX: "auto", scrollbarWidth: "none" }}>
        {shuffled.map(ad => {
          ...
          const prods = (bannerProds[ad.id!] || []).slice(0, 2);
          return (
            <div key={ad.id} onClick={() => ad.vendorId && onVendorClick?.(ad.vendorId)}
              ...
              <div style={{ padding: "14px 16px", display: "flex", alignItems: "center", gap: 12, position: "relative", zIndex: 1 }}>
                <div style={{ width: 42, height: 42, borderRadius: 12, background: "rgba(255,255,255,0.15)", backdropFilter: "blur(6px)", border: "1.5px solid rgba(255,255,255,0.25)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, overflow: "hidden" }}>
                  {bd.logoUrl ? <img src={bd.logoUrl} alt="logo" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <RiStore2Line size={18} color={s.titleColor} />}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 10, color: s.subColor, marginBottom: 7 }}>{bd.tagline}</div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 4, flexShrink: 0 }}>
                    {prods.map((p, i) => (
                      <div key={i} style={{ width: 34, height: 34, borderRadius: 8, overflow: "hidden", background: "rgba(255,255,255,0.12)", border: "1px solid rgba(255,255,255,0.2)" }}>
                        {p.img ? <img src={p.img} alt={p.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center" }}><FiPackage size={12} color={s.titleColor} /></div>}
                      </div>
                    ))}
                  </div>
                )}
  ...
  return (
    <div className="hp-pc">
      ...
      <div className="hp-pb">
        ...
        {p.vendorName && <div style={{ display: "flex", alignItems: "center", gap: 3, marginBottom: 3, minWidth: 0 }}><span className="hp-ps">{p.vendorName}</span></div>}
        <div style={{ marginTop: "auto" }}>
          <span style={{ color: ACCENT, fontWeight: 900, fontSize: 13, fontFamily: "'Syne',sans-serif", display: "block", marginBottom: 7 }}>₦{String(p.price || 0)}</span>
    ...
    <div style={{ marginBottom: 32 }}>
      ...
      {loading ? (
        ...
        <div className="hp-hscr">
          {products.map((p, i) => <TrendingAdCard key={p.id + i} p={p} />)}
        </div>
      )}
    </div>
  );
    ...
    (async () => {
      ...
      try {
        ...
        if (product.vendorId) {
          ...
          if (vs.exists()) {
            const d = vs.data();
            v = { name: d.businessName || d.storeName || product.store, logo: d.logo || d.coverImage, verified: d.verified, rating: d.rating, reviewCount: d.reviewCount, deliveryTime: d.deliveryTime || "15–35 mins", address: d.address || d.city };
          }
        }
        setVendor(v);
      ...
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 6001, maxHeight: "93vh", overflowY: "auto", scrollbarWidth: "none", borderRadius: "26px 26px 0 0", background: D.bg, animation: open ? "sn-up .35s cubic-bezier(.32,1,.4,1)" : "sn-dn .3s ease forwards", willChange: "transform" }}>
        ...
        <div style={{ position: "relative", height: 245, overflow: "hidden" }}>
          ...
          <div style={{ position: "absolute", top: 14, right: 14, display: "flex", gap: 8 }}>
            <button onClick={() => setLiked(v => !v)} style={{ width: 36, height: 36, borderRadius: "50%", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "white", background: liked ? "rgba(239,68,68,.85)" : "rgba(0,0,0,.48)", backdropFilter: "blur(8px)" }}>
              <FiHeart size={14} fill={liked ? "white" : "none"} />
            </button>
            <button onClick={close} style={{ width: 36, height: 36, borderRadius: "50%", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "white", background: "rgba(0,0,0,.48)", backdropFilter: "blur(8px)" }}>
              <FiX size={14} />
            </button>
          </div>
        ...
        <div style={{ padding: "20px 20px 0" }}>
          ...
          {dp.description && (
            <>
              <p style={{ fontSize: 11, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".8px", color: D.sub, marginBottom: 8 }}><FiBox size={10} /> About this product</p>
              <p style={{ fontSize: 14, lineHeight: 1.75, fontWeight: 500, color: D.txt, marginBottom: 16 }}>{dp.description}</p>
            </>
          )}
          {dots.length > 0 && (
            <>
              <p style={{ fontSize: 11, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".8px", color: D.sub, marginBottom: 10 }}>Key Highlights</p>
              <div style={{ display: "flex", flexDirection: "column", gap: 7, marginBottom: 16 }}>
                {dots.map((h, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10, fontSize: 13, fontWeight: 600, color: D.txt }}>
          ...
          {dp.careInfo && (
            <div style={{ background: "rgba(255,107,0,.07)", border: "1px solid rgba(255,107,0,.18)", borderRadius: 13, padding: "11px 14px", marginBottom: 16, fontSize: 12, fontWeight: 600, color: D.sub, lineHeight: 1.6 }}>
              Care Info: {dp.careInfo}
            </div>
          )}
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", margin: "15px 0 10px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 4, background: "rgba(255,107,0,.1)", borderRadius: 13, padding: 4, border: "1.5px solid rgba(255,107,0,.2)" }}>
              <button disabled={qty <= 1} onClick={() => setQty(v => Math.max(1, v - 1))} style={{ width: 34, height: 34, borderRadius: 9, border: "none", background: "transparent", color: ACCENT, cursor: qty <= 1 ? "not-allowed" : "pointer", opacity: qty <= 1 ? 0.35 : 1, display: "flex", alignItems: "center", justifyContent: "center" }}><FiMinus size={12} /></button>
              <span style={{ fontFamily: "'Syne',sans-serif", fontSize: 16, fontWeight: 900, color: ACCENT, minWidth: 30, textAlign: "center" }}>{qty}</span>
              <button onClick={() => setQty(v => v + 1)} style={{ width: 34, height: 34, borderRadius: 9, border: "none", background: "transparent", color: ACCENT, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}><FiPlus size={12} /></button>
            </div>
          </div>
    ...
    <div className="hp-pc" onClick={() => onOpen(p)}>
      ...
      <div className="hp-pb">
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 3, marginBottom: 3, minWidth: 0 }}>
          <span className="hp-ps">{p.store}</span>
          {p.vendorVerified && <RiVerifiedBadgeFill size={11} color="#3b82f6" style={{ flexShrink: 0 }} />}
        </div>
        ...
        <div style={{ marginTop: "auto" }}>
          <span style={{ color: ACCENT, fontWeight: 900, fontSize: 13, fontFamily: "'Syne',sans-serif", display: "block", marginBottom: 7 }}>₦{p.price}</span>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
...
// ─── Categories Section ───────────────────────────────────────────────────────
// NOTE: Send & Pickup is a service button shown separately, NOT in this grid
function CategoriesSection({ navigate }: { navigate: (path: string) => void }) {
  const [showAll, setShowAll] = React.useState(false);
  const [visibleLimit, setVisibleLimit] = React.useState(6);

  React.useEffect(() => {
    function updateLimit() {
      if (window.innerWidth >= 1024) setVisibleLimit(13);
      else if (window.innerWidth >= 768) setVisibleLimit(8);
      else setVisibleLimit(6);
    }
    updateLimit();
  ...
  const catsToShow = showAll ? CATEGORY_IDS : CATEGORY_IDS.slice(0, visibleLimit);
  const hasMore = CATEGORY_IDS.length > visibleLimit;
  ...
  return (
    <>
      <div className="hp-slbl">
        ...
        {hasMore && (
          <button className="hp-seeall" onClick={() => setShowAll(v => !v)}>
            {showAll ? "Show less" : "See all"}
          </button>
        )}
      </div>

      {/* Send & Pickup service shortcut — shown above the category grid */}
      <div style={{ padding: "0 18px 14px" }}>
        <button
          onClick={() => navigate("/send-pickup")}
          style={{
            width: "100%", display: "flex", alignItems: "center", gap: 12,
            padding: "13px 16px", borderRadius: 15,
            background: "linear-gradient(135deg,rgba(255,107,0,0.1),rgba(255,140,0,0.06))",
            border: "1.5px solid rgba(255,107,0,0.25)",
            cursor: "pointer", fontFamily: "inherit", textAlign: "left",
          }}
        >
          <div style={{ width: 38, height: 38, borderRadius: 11, background: "#FF6B00", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            <MdDirectionsBike size={20} color="white" />
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 13, fontWeight: 800, color: "var(--ht)" }}>Send & Pickup</div>
            <div style={{ fontSize: 11, color: "var(--hs)", fontWeight: 600, marginTop: 1 }}>Parcels, documents, errands & airport rides</div>
          </div>
          <FiChevronRight size={16} color="#FF6B00" />
        </button>
      </div>

      <div className="hp-catgrid">
        {catsToShow.map((id: string) => {
          const info = CAT[id];
          if (!info) return null;
          return (
            <button key={id} className="hp-catcard" onClick={() => navigate(`/category/${id}`)}>
...
export default function Homepage() {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      const hydrated = raw.map(p => {
        let verified = false, storeName = p.store;
        if (p.vendorId && vendorMap[p.vendorId]) {
          verified = vendorMap[p.vendorId].verified;
          if (!storeName || storeName === "Store") storeName = vendorMap[p.vendorId].name;
        } else if (p.vendorName && vendorMap[`name:${p.vendorName}`]) {
          verified = vendorMap[`name:${p.vendorName}`].verified;
          if (!storeName || storeName === "Store") storeName = vendorMap[`name:${p.vendorName}`].name;
        }
      ...
      for (const p of all) {
        // Only group into real categories (no sendpickup)
        if (!(p.category in CAT)) continue;
  ...
  const addProd = useCallback(async (p: Product, qty = 1) => {
    ...
    if (p.vendorId) {
      try {
        const s = await getDoc(doc(db, "vendors", p.vendorId));
        if (s.exists()) { const d = s.data(); vLat = typeof d.lat === "number" ? d.lat : undefined; vLng = typeof d.lng === "number" ? d.lng : undefined; }
      } catch {}
    }
    for (let i = 0; i < qty; i++)
      addToCart({ name: p.name, price: `₦${p.price}`, img: p.img ?? FALLBACK, vendorName: p.vendorName, vendorId: p.vendorId || undefined, vendorLat: vLat, vendorLng: vLng });
  }, [addToCart]);
  ...
  return (
    <div className="hp-root">
      {maintenanceBanner?.active && !bannerDismissed && (
        <div style={{ background: "linear-gradient(90deg,#7f1d1d,#991b1b)", color: "white", padding: "12px 18px", display: "flex", alignItems: "center", gap: 10, fontSize: 13, fontWeight: 700, position: "sticky", top: 0, zIndex: 9999 }}>
          🔧 <span style={{ flex: 1 }}>{maintenanceBanner.message}</span>
          <button onClick={dismissBanner} style={{ background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.3)", borderRadius: 8, color: "white", cursor: "pointer", padding: "4px 10px", fontSize: 12, fontWeight: 700, flexShrink: 0 }}>✕ Dismiss</button>
        </div>
      )}
        ...
        <main className="hp-main">
          ...
          <div className="hp-topbar">
            ...
            <button className="hp-locpill" onClick={() => setLocModal(true)}>
              <FiMapPin size={11} color={ACCENT} />
              <span>{locLabel.slice(0, 26)}</span>
              <FiChevronRight size={10} color="#666" />
            </button>
          </div>
          ...
          <div className="hp-dhdr">
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <button className="hp-locpill desk" onClick={() => setLocModal(true)}>
                <FiMapPin size={13} color={ACCENT} /><span>{locLabel}</span>
              </button>
              <button className="hp-cartdesk" onClick={() => navigate("/cart")}>
                <FiShoppingCart size={16} /> Cart
                {cartCount > 0 && <span className="hp-bdgi">{cartCount}</span>}
              </button>
            </div>
          </div>
          ...
          {/* Categories + Send&Pickup service button */}
          {!search && <CategoriesSection navigate={navigate} />}

          {/* Trending ads */}
          {!search && <div style={{ marginTop: 24 }}><HomepageTrendingAds dark={dark} /></div>}
            ...
            filtered.map((sec, secIdx) => (
              <div key={sec.id}>
                <div className="hp-slbl">
                  ...
                  <span>{sec.title}</span>
                  <button className="hp-seeall" onClick={() => navigate(`/category/${sec.id}`)}>See all →</button>
                </div>
                <div className="hp-hscr">
...
.phone-alert-close{background:none;border:none;color:var(--hs);display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0;padding:4px;border-radius:6px;}
...
.hp-logo .hp-logo-ja{color:var(--ht);font-style:normal;font-size:19px;font-family:'Nunito',sans-serif;font-weight:900;line-height:1;display:inline-block;vertical-align:bottom;}
...
.hp-locpill.desk{padding:10px 20px;font-size:13px;border-radius:50px;border:1.5px solid var(--hbrd);max-width:360px;flex:1;}
.hp-cartdesk{display:flex;align-items:center;gap:7px;background:linear-gradient(135deg,#FF6B00,#FF8C00);border:none;border-radius:12px;padding:10px 18px;color:white;font-family:'Nunito',sans-serif;font-size:14px;font-weight:800;cursor:pointer;position:relative;box-shadow:0 4px 16px rgba(255,107,0,.35);}
...
.hp-iconbtn{width:38px;height:38px;background:var(--hcard);border:1px solid var(--hbrd);border-radius:10px;display:flex;align-items:center;justify-content:center;cursor:pointer;color:var(--hs);}
.hp-srow{display:flex;gap:10px;padding:16px 18px 0;}
@media(min-width:768px){.hp-srow{padding:22px 48px 0;}}
.hp-sbar{flex:1;display:flex;align-items:center;gap:9px;background:var(--hsurf);border:1.5px solid var(--hbrd);border-radius:13px;padding:13px 18px;}
...
.hp-stat{background:var(--hsurf);border:1px solid var(--hbrd);border-radius:18px;padding:22px 16px;display:flex;flex-direction:column;align-items:center;gap:6px;text-align:center;}
...
.hp-seeall{margin-left:auto;color:#FF6B00;font-size:12px;font-weight:700;background:none;border:none;cursor:pointer;font-family:'Nunito',sans-serif;padding:5px 12px;border-radius:8px;flex-shrink:0;}
...
.hp-fav{width:30px;height:30px;border-radius:8px;border:1.5px solid var(--hbrd);background:var(--hsurf);color:var(--hs);display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0;}
