...
// Rider payout dashboard with two tabs:
//   1. Paystack — existing Paystack wallet balance & earnings
...
import {
  ...
  FiArrowUp, FiShield, FiRefreshCw,
} from "react-icons/fi";
...
// ─── SHARED: BANK LINK FORM ───────────────────────────────────────────────────
function BankLinkForm({
  onLink, loading: savingBank, error, onClearError,
}: {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      } catch { setVerifyError("Verification failed. Check connection."); }
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: "#13131a", border: "1.5px solid rgba(255,255,255,0.1)", borderRadius: 24, padding: 28, width: "100%", maxWidth: 400, boxShadow: "0 32px 80px rgba(0,0,0,0.6)" }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {quickAmounts.map(a => (
              <button key={a} onClick={() => { setAmount(a.toLocaleString("en-NG")); setError(""); }}
                style={{ padding: "7px 14px", borderRadius: 20, cursor: "pointer", border: `1.5px solid ${num === a ? "rgba(255,107,0,.5)" : "rgba(255,255,255,0.1)"}`, background: num === a ? "rgba(255,107,0,.12)" : "transparent", color: num === a ? ACCENT : "rgba(232,232,240,0.5)", fontSize: 13, fontWeight: 700 }}>
                ₦{a.toLocaleString()}
              </button>
            ))}
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <button onClick={handleWithdraw} disabled={loading || !num}
            style={{ flex: 2, padding: "12px", borderRadius: 12, border: "none", background: loading || !num ? "#1e1e2c" : "linear-gradient(135deg,#FF6B00,#FF9A00)", color: loading || !num ? "#555" : "white", fontFamily: "'Syne',sans-serif", fontSize: 14, fontWeight: 900, cursor: loading || !num ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, boxShadow: loading || !num ? "none" : "0 4px 20px rgba(255,107,0,0.4)" }}>
...
// ─── PAYSTACK TAB (existing earnings view) ────────────────────────────────────
function PaystackTab({ riderId, fns }: { riderId: string; fns: ReturnType<typeof getFunctions> }) {
  ...
  useEffect(() => {
    if (!riderId) return;
    const u1 = onSnapshot(doc(db, "riderWallets", riderId), snap => setBalance(snap.exists() ? (snap.data()!.balance ?? 0) : 0));
    const u2 = onSnapshot(query(collection(db, "riderWalletTransactions"), where("riderId", "==", riderId), orderBy("createdAt", "desc"), limit(30)), snap => setTxs(snap.docs.map(d => ({ id: d.id, ...d.data() })) as RiderWalletTx[]));
    const u3 = onSnapshot(doc(db, "riderBankAccounts", riderId), snap => setBankAccount(snap.exists() ? snap.data() : null));
    return () => { u1(); u2(); u3(); };
  }, [riderId]);
  ...
  const handleLinkBank = async (params: { bank: PaystackBank; account_number: string; account_name: string }) => {
    ...
    try {
      await httpsCallable(fns, "createRiderPaystackRecipient")({ account_number: params.account_number, bank_code: params.bank.code, account_name: params.account_name, bank_name: params.bank.name });
  ...
  return (
    <>
      ...
      {showWithdraw && bankAccount && <WithdrawModal balance={balance} bankName={bankAccount.bank_name} accountLast4={bankAccount.account_number.slice(-4)} onClose={() => setShowWithdraw(false)} onWithdraw={handleWithdraw} />}
      ...
      <div style={{ background: "linear-gradient(135deg,#FF6B00,#FF9A00)", borderRadius: 22, padding: "24px 22px", marginBottom: 20, position: "relative", overflow: "hidden", boxShadow: "0 12px 40px rgba(255,107,0,.35)" }}>
        ...
        <div style={{ fontSize: 12, color: "rgba(255,255,255,.7)", marginTop: 4, marginBottom: 18 }}>Total earned: ₦{totalIn.toLocaleString("en-NG")}</div>
        {bankAccount ? (
          <button onClick={() => setShowWithdraw(true)} disabled={balance < 100} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, background: "white", color: ACCENT, border: "none", borderRadius: 14, padding: "13px 24px", width: "100%", fontFamily: "'Syne',sans-serif", fontSize: 14, fontWeight: 900, cursor: balance < 100 ? "not-allowed" : "pointer", opacity: balance < 100 ? 0.6 : 1, boxShadow: "0 4px 16px rgba(0,0,0,.15)" }}>
...
// ─── WALLET TAB (split-payment wallet) ───────────────────────────────────────
function WalletTab({ riderId, fns }: { riderId: string; fns: ReturnType<typeof getFunctions> }) {
  const [balance, setBalance] = useState(0);
  const [txs, setTxs] = useState<RiderWalletTx[]>([]);
  const [bankAccount, setBankAccount] = useState<any>(null);
  const [savingBank, setSavingBank] = useState(false);
  ...
  useEffect(() => {
    if (!riderId) return;
    // Wallet balance from order splits — stored in riderSplitWallets
    const u1 = onSnapshot(doc(db, "riderSplitWallets", riderId), snap => setBalance(snap.exists() ? (snap.data()!.balance ?? 0) : 0));
    // Split wallet transactions
    const u2 = onSnapshot(query(collection(db, "riderSplitWalletTransactions"), where("riderId", "==", riderId), orderBy("createdAt", "desc"), limit(30)), snap => setTxs(snap.docs.map(d => ({ id: d.id, ...d.data() })) as RiderWalletTx[]));
    // Shared bank account
    const u3 = onSnapshot(doc(db, "riderBankAccounts", riderId), snap => setBankAccount(snap.exists() ? snap.data() : null));
    return () => { u1(); u2(); u3(); };
  }, [riderId]);
  ...
  const handleWithdraw = async (amount: number) => {
    // Uses riderSplitWalletWithdraw cloud function
    const res = await httpsCallable(fns, "riderSplitWalletWithdraw")({ amount }) as any;
    addToast(`₦${amount.toLocaleString("en-NG")} sent to your bank 🎉`);
    setBalance(res.data.newBalance);
  };
  ...
  return (
    <>
      ...
      {/* Balance card — purple tint to distinguish from Paystack tab */}
      <div style={{ background: "linear-gradient(135deg,#6366f1,#8b5cf6)", borderRadius: 22, padding: "24px 22px", marginBottom: 20, position: "relative", overflow: "hidden", boxShadow: "0 12px 40px rgba(99,102,241,.35)" }}>
        <div style={{ position: "absolute", top: -30, right: -30, width: 150, height: 150, borderRadius: "50%", background: "rgba(255,255,255,.08)", pointerEvents: "none" }} />
        ...
        <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 22, fontWeight: 900, color: "white", letterSpacing: "-1px", lineHeight: 1 }}>₦{balance.toLocaleString("en-NG", { minimumFractionDigits: 2 })}</div>
        <div style={{ fontSize: 12, color: "rgba(255,255,255,.7)", marginTop: 4, marginBottom: 18 }}>Earned from deliveries: ₦{totalIn.toLocaleString("en-NG")}</div>
        <div style={{ fontSize: 11, color: "rgba(255,255,255,.6)", marginBottom: 18 }}>💡 These are your delivery split earnings — withdraw anytime</div>
        {bankAccount ? (
          <button onClick={() => setShowWithdraw(true)} disabled={balance < 100} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, background: "white", color: "#6366f1", border: "none", borderRadius: 14, padding: "13px 24px", width: "100%", fontFamily: "'Syne',sans-serif", fontSize: 14, fontWeight: 900, cursor: balance < 100 ? "not-allowed" : "pointer", opacity: balance < 100 ? 0.6 : 1, boxShadow: "0 4px 16px rgba(0,0,0,.15)" }}>
            <FiArrowDown size={16} /> Withdraw to Bank
          </button>
        ) : (
          <div style={{ background: "rgba(255,255,255,.15)", borderRadius: 12, padding: "10px 14px", fontSize: 12, fontWeight: 700, color: "rgba(255,255,255,.9)" }}>🏦 Link a bank account below to withdraw</div>
      ...
      {txs.length === 0 ? (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10, padding: "36px 20px", background: "rgba(255,255,255,0.03)", border: "1.5px dashed rgba(255,255,255,0.08)", borderRadius: 18, color: "rgba(232,232,240,0.4)" }}>
          ...
          <span style={{ fontSize: 12 }}>Complete deliveries to earn split payments</span>
        </div>
      ) : (
        <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 18, overflow: "hidden" }}>
          {txs.map((tx, i) => (
...
export default function RiderPayoutsPage() {
  ...
  const tabs = [
    { key: "paystack" as const, label: "Paystack", icon: "💳" },
  ...
  return (
    <>
      ...
      <div style={{ padding: "0 20px 120px" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 20, background: "rgba(255,255,255,0.04)", borderRadius: 16, padding: 6, border: "1px solid rgba(255,255,255,0.08)" }}>
          {tabs.map(tab => (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)}
              style={{ flex: 1, padding: "11px 0", borderRadius: 12, border: "none", cursor: "pointer", fontFamily: "'Syne',sans-serif", fontSize: 13, fontWeight: 800, transition: "all 0.2s", background: activeTab === tab.key ? (tab.key === "paystack" ? "linear-gradient(135deg,#FF6B00,#FF9A00)" : "linear-gradient(135deg,#6366f1,#8b5cf6)") : "transparent", color: activeTab === tab.key ? "white" : "rgba(232,232,240,0.4)", boxShadow: activeTab === tab.key ? "0 4px 16px rgba(0,0,0,0.3)" : "none", display: "flex", alignItems: "center", justifyContent: "center", gap: 7 }}>
        ...
        {activeTab === "paystack"
          ? <PaystackTab riderId={riderId} fns={fns} />
          : <WalletTab riderId={riderId} fns={fns} />
        }
      </div>
    </>
  );
